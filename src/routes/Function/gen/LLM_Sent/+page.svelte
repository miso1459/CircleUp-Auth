<script lang="ts">
	...
	function handleSearch() {
		...
		window.location.href = `/Function/gen/LLM_Sent${params}`;
	}
...
<div class="space-y-6 p-6">
	...
	<div class="space-y-1">
		...
		<p class="text-muted-foreground text-sm">
			구글 Gemini API를 활용하여 입력한 프롬프트를 바탕으로 새로운 문장들을 생성하고 데이터베이스에 저장합니다.
		</p>
	...
	<div class="grid gap-6 lg:grid-cols-3">
		...
		<Card.Root class="lg:col-span-1 border-muted">
			<Card.Header>
				...
				<Card.Description>언어 선택 및 생성 기준을 설정합니다.</Card.Description>
			</Card.Header>
			<Card.Content>
		...
		<Card.Root class="lg:col-span-2 border-muted">
			<Card.Header>
				...
				<Card.Description>Gemini가 어떤 종류의 문장들을 생성할지 구체적으로 지시합니다.</Card.Description>
			</Card.Header>
			<Card.Content class="space-y-3">
				<Textarea
					...
					name="prompt"
					bind:value={prompt}
					placeholder="예: 초급 수준의 여행 필수 영어 회화 문장 10개 만들어줘."
