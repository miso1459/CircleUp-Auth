<script lang="ts">
	...
	function handleSearch() {
		...
		window.location.href = `/LLM_SendAll${params}`;
	}

	async function handleDelete(id: number) {
		if (!confirm('삭제하시겠습니까?')) return;
		try {
			const res = await fetch('/LLM_SendAll', {
				method: 'DELETE',
...
<div class="space-y-6 p-6">
	...
	<div class="space-y-1">
		...
		<p class="text-muted-foreground text-sm">
			Google Gemini API를 활용한 문장 태그 생성 및 관리 도구입니다.
		</p>
	</div>

	<!-- 접이식 LLM Tag 영역 -->
	<Collapsible.Root bind:open={tagOpen} class="border rounded-lg">
		<Collapsible.Trigger class="flex w-full items-center justify-between rounded-lg px-4 py-3 text-sm font-medium hover:bg-muted/50 transition-colors">
			<span class="flex items-center gap-2">
				<Database class="size-4 text-indigo-500" />
				LLM Tag Generation
			</span>
			<ChevronDown class="size-4 transition-transform duration-200 {tagOpen ? 'rotate-180' : ''}" />
		</Collapsible.Trigger>
		<Collapsible.Content class="px-4 pb-4 pt-0">
			<div class="space-y-6 pt-2">
				<!-- Gemini 경고 -->
				{#if !data.geminiConfigured}
					<div class="bg-destructive/10 border-destructive/20 text-destructive flex items-start gap-2 rounded-lg border p-3.5 text-sm">
