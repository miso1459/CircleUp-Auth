...
import { sentences, config } from '$lib/server/db/schema';
import { eq, inArray } from 'drizzle-orm';
import { generateTTS } from '$lib/server/tts';
import fs from 'fs';
import path from 'path';

export async function getTTSLoadData({ locals }: { locals: App.Locals }) {
...
export const ttsActions = {
	processTTS: async ({ request, locals }: { request: Request; locals: App.Locals }) => {
		if (locals.user?.role !== 'admin') {
	...
	processAllTTS: async ({ request, locals }: { request: Request; locals: App.Locals }) => {
		if (locals.user?.role !== 'admin') {
			return fail(403, { error: 'Unauthorized: Admin access required' });
		}
